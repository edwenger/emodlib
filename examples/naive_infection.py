...
if __name__ == "__main__":
    configure_from_file(os.path.join(os.path.dirname(__file__), "..", "tests", "config.yml"))
    ...
    df.plot(x='days', y='parasite_density', ax=ax, color='firebrick', marker='o', alpha=0.5, legend=False)
    df.plot(x='days', y='gametocyte_density', ax=ax, color='darkgreen', marker='o', alpha=0.5, legend=False)
    ax.set(ylim=(1e-3, 1e6), xlim=(0, 80))
    ax.set(yscale="log")

    # draw infectiousness measurement points
    ax.scatter(df.days, y=[4e5]*len(df),
               s=1+100*df.infectiousness, c=100*df.infectiousness,
               cmap='Greens', vmin=0, vmax=100,
               lw=0.5, edgecolors='darkgreen')

    # draw fever measurement points
    ax.scatter(df.days, y=[8e5]*len(df),
               s=20 * (df.fever_temperature - 37) + 1, c=df.fever_temperature - 37,
               cmap='Reds', vmin=0, vmax=4,
               lw=0.5, edgecolors='firebrick')
